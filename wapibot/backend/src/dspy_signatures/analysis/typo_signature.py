...
class TypoDetectionSignature(dspy.Signature):
    ...
    user_message: str = dspy.InputField(desc="User message to check for typos")
    field_context: str = dspy.InputField(
        desc="Field being extracted (e.g., 'name', 'vehicle_brand', 'email')"
    )
    extracted_value: str = dspy.InputField(desc="The value extracted from the message")

    # Typo detection output
    has_typo: str = dspy.OutputField(desc="Whether typo detected: 'yes' or 'no'")
