...
class ResponseProposalSignature(dspy.Signature):
    ...
    proposed_response = dspy.OutputField(desc="Optimal response message")
    tone = dspy.OutputField(desc="Response tone: friendly|professional|empathetic|firm")
    confidence = dspy.OutputField(desc="Confidence in this response 0.0-1.0")
    reasoning = dspy.OutputField(desc="Why this response is appropriate")
