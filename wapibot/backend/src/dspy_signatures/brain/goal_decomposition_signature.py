...
class GoalDecompositionSignature(dspy.Signature):
    ...
    current_state = dspy.InputField(desc="Current booking state")

    main_goal = dspy.OutputField(desc="Primary goal identified")
    sub_goals = dspy.OutputField(desc="List of sub-goals/steps needed (JSON array)")
    required_data = dspy.OutputField(desc="Data needed to achieve goals (JSON array)")
