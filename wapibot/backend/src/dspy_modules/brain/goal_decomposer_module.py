...
import dspy
from dspy_signatures.brain.goal_decomposition_signature import (
...
class GoalDecomposer(dspy.Module):
    ...
    def forward(
        self, user_message: str, predicted_intent: str, current_state: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            result = self.decomposer(user_message=user_message, current_state=state_str)

            # Parse sub-goals (comma-separated string to list)
            sub_goals = [
                goal.strip() for goal in result.sub_goals.split(",") if goal.strip()
            ]

            # Parse required info
            required_info = [
                info.strip() for info in result.required_info.split(",") if info.strip()
            ]
