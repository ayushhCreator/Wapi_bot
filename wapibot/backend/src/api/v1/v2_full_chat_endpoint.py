...
router = APIRouter(prefix="/api/v1", tags=["V2 Chat"])


@router.post("/v2/chat", response_model=ChatResponse)
async def process_v2_chat(request: ChatRequest) -> ChatResponse:
    """
    ...
    **Edge Case Example (low confidence → retroactive scan):**
    ...
    try:
        logger.info(
            f"[V2] Processing chat: {request.conversation_id} - "
        ...
        logger.info(f"[V2] Response: {response.message[:50]}...")
        logger.info(f"[V2] Extracted: {result.get('customer')}")

        return response

    except Exception as e:
        logger.error(f"[V2] Chat processing failed: {e}", exc_info=True)
        raise HTTPException(
            status_code=500,
            detail=f"V2 chat processing error: {str(e)}"
        )
