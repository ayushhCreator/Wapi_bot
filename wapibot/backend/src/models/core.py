...
class ValidationResult(BaseModel):
    ...
    confidence_score: float = Field(
        ge=0.0, le=1.0, default=1.0, description="Confidence in extracted data"
    )
...
class ExtractionMetadata(BaseModel):
    ...
    extraction_method: Literal[
        "direct", "chain_of_thought", "fallback", "rule_based", "dspy"
    ...
    processing_time_ms: float = Field(
        ge=0.0, default=0.0, description="Processing time in milliseconds"
    )
...
class ConfidenceThresholdConfig(BaseModel):
    ...
    minimum_acceptable: float = Field(
        ge=0.0, le=1.0, default=0.6, description="Minimum acceptable confidence"
    )
    high_confidence: float = Field(
        ge=0.0, le=1.0, default=0.85, description="High confidence threshold"
    )
    require_human_review_below: float = Field(
        ge=0.0, le=1.0, default=0.5, description="Require human review below this"
    )
