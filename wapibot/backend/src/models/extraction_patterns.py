...
class DatePattern(BaseModel):
    ...
    requires_weekday: bool = Field(
        default=False, description="Whether pattern requires weekday calculation"
    )
    weekday_name: str = Field(
        default="", description="Weekday name for weekday-based patterns"
    )
...
DATE_PATTERNS: List[DatePattern] = [
    DatePattern(
        pattern_name="today", regex_patterns=[r"\b(today|aaj|aaj)\b"], day_offset=0
    ),
    DatePattern(
