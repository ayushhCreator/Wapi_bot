...
class ChatbotResponse(BaseModel):
    ...
    message: str = Field(
        ..., min_length=1, max_length=1000, description="Response message to user"
    )
    should_proceed: bool = Field(
        ..., description="Whether conversation should continue"
    )
    extracted_data: Optional[Dict[str, Any]] = Field(
        None, description="Data extracted from user message"
    )
    ...
    suggestions: Optional[Dict[str, Any]] = Field(
        None, description="Conversation handling suggestions"
    )
    ...
    should_confirm: bool = Field(default=False, description="Show confirmation screen")
    scratchpad_completeness: float = Field(
        default=0.0, ge=0.0, le=100.0, description="Data completeness (0-100%)"
    )
    scratchpad: Optional[Dict[str, Any]] = Field(
        None, description="Current scratchpad state"
    )
    state: str = Field(default="greeting", description="Current conversation state")
    data_extracted: bool = Field(
        default=False, description="New data extracted this turn"
    )
    typo_corrections: Optional[Dict[str, str]] = Field(
        default=None, description="Suggested typo corrections"
    )
    service_request_id: Optional[str] = Field(
        default=None, description="Service request ID if created"
    )
    service_request: Optional[Dict[str, Any]] = Field(
        default=None, description="Full service request details"
    )
...
class ExtractionResult(BaseModel):
    ...
    success: bool = Field(..., description="Extraction successful")
